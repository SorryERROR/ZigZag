...
namespace SZZ
{
	...
	/// </summary>
	public class SZZIRBlock : MonoBehaviour
	{
		...
		{
			if (SZZSounds == null)
				SZZSounds = GameObject.FindGameObjectWithTag("Sound").GetComponent<AudioSource>();
		...
		{	
			...
			if ( other.tag == touchTargetTag )
			{
				...
				if (soundHit )    SZZSounds.PlayOneShot(soundHit);
			}
		}
	}
}
